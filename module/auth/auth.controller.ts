...
class AuthController {
  async signIn(req: Request<BodyObject, BodyObject, TLoginDTO>, res: Response) {
    const userCreds = req.body;

    const validData = zodValidation(signInSchema, userCreds, "auth");

    const loginData: TLoginResponseDTO | null = await authService.signIn(
      validData
    );
  ...
  async signUp(
    ...
    const validData = zodValidation(signUpSchema, { ...userData }, "auth");
    ...
    const userForTrans: TSignUpResponseDTO = removeKey(registeredUser, [
      "password",
    ]);
